...
function createStyles(T, isDesktop, isMobile) {
  return StyleSheet.create({
    ...
    reviewCard: {
      ...
      borderRadius: 16,
      borderWidth: 1,
      borderColor: 'rgba(254,94,3,0.15)',
      padding: 14,
      gap: 4,
    },
    reviewCardHeader: {
...
export default function DashboardScreen({ navigation }) {
  ...
  const MOCK_NOTIFICATIONS = useMemo(() => [
    { id: '__mock_1', descricao: 'Salário', categoria: 'Outros', valor: 3500.00, tipo: 'entrada', data: '01/06/2026', origin: { type: 'notification', bank: 'Itaú' } },
    { id: '__mock_2', descricao: 'iFood', categoria: 'Alimentação', valor: 47.90, tipo: 'saída', data: '01/06/2026', origin: { type: 'notification', bank: 'Nubank' } },
    { id: '__mock_3', descricao: 'Mercado Extra', categoria: 'Alimentação', valor: 213.50, tipo: 'saída', data: '01/06/2026', origin: { type: 'notification', bank: 'Bradesco' } },
  ], []);
        ...
        {notificationCount > 0 && (
          <View style={styles.reviewSection}>
            ...
            <ScrollView
              ...
              onScroll={(e) => {
                const idx = Math.round(e.nativeEvent.contentOffset.x / (winW - (isDesktop ? 80 : 40)));
                setReviewPage(idx);
              }}
                ...
                return (
                  <View key={tx.id} style={[styles.reviewCard, { width: winW - (isDesktop ? 80 : 40) }]}>
                    <View style={styles.reviewCardHeader}>
                      <Bell size={14} color={T.orange} weight="fill" />
                      <Text style={styles.reviewCardBank} numberOfLines={1}>
                    ...
                    <View style={styles.reviewActionsRow}>
                      <TouchableOpacity
                        style={styles.reviewBtnConfirm}
                        onPress={() => confirmNotification(tx)}
                        activeOpacity={0.8}
                      >
                        <Text style={styles.reviewBtnText}>Confirmar</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={styles.reviewBtnDismiss}
                        onPress={() => dismissNotification(tx)}
                        activeOpacity={0.8}
                      >
                        <Text style={styles.reviewBtnText}>Descartar</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                );
